...
public interface SmsRecordsDao extends JpaRepository<SmsRecords, Integer>, CrudRepository<SmsRecords, Integer> {
    ...
    @Query(value = "SELECT * FROM `sms_records` where number =:phone and time > :time and status = 0 order by id desc ", nativeQuery = true)
    SmsRecords findByNumberCodeFromTime(@Param(value = "phone")String phone, @Param(value = "time")long time);
    @Query(value = "SELECT * FROM `sms_records` where number =:phone and code = :code and time > :time and status =0 order by id asc ", nativeQuery = true)
    SmsRecords findByNumberCodeFirst(@Param(value = "phone")String phone, @Param(value = "code")String code, @Param(value = "time")long time);
    @Query(value = "SELECT * FROM `sms_records` where number =:phone and code = :code order by id desc ", nativeQuery = true)
